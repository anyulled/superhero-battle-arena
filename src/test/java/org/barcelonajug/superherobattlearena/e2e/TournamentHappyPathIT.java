...
/**
 * End-to-end integration test for the complete tournament happy path. Tests the
 * full workflow from
...
class TournamentHappyPathIT {
        ...
        private void verifySubmissionExists(UUID teamId, int roundNo) throws Exception {
                mockMvc
                                .perform(
                                                get("/api/rounds/{roundNo}/submission", roundNo).param("teamId",
                                                                teamId.toString()))
                                .andExpect(status().isOk());
        }
        ...
        private JsonNode getMatch(UUID matchId) throws Exception {
                MvcResult result = mockMvc.perform(get("/api/matches/{matchId}", matchId)).andExpect(status().isOk())
                                .andReturn();
